...
class BertClassifier(th.nn.Module):
    ...
    def forward(self, input_ids, position_ids, attention_mask):
        cls_feats = self.bert_model(input_ids=input_ids, position_ids=position_ids, attention_mask=attention_mask)[0][:, 0]
...
class BertGCN(th.nn.Module):
    ...
    def forward(self, g, idx):
        input_ids, position_ids, attention_mask = \
            g.ndata['input_ids'][idx], g.ndata['position_ids'][idx], g.ndata['attention_mask'][idx]
        if self.training:
            cls_feats = self.bert_model(
                input_ids=input_ids, position_ids=position_ids, attention_mask=attention_mask)[0][:, 0]
